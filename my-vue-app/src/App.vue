<!-- Einfache Ein-Datei-Webseite mit Vue 3 (CDN) und Darkmode.
     - Zwei Seiten: Main und Spam
     - Top-Banner
     - Content-Bereich mit Post-Boxen
     - Funktionen: Like, Dislike, als Spam markieren
     - Darkmode Toggle mit dunklem Hintergrund und vibriereneden Textfarben -->
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    body{font-family:Consolas, monospace; margin:0; background:var(--bg); color:var(--text); transition: background 0.3s, color 0.3s}

    header{background:linear-gradient(90deg,#0ea5e9,#7c3aed); color:white; padding:18px 24px; display:flex; align-items:center; justify-content:space-between}
    .brand{font-weight:700; font-size:1.1rem}
    nav{display:flex; gap:8px}
    .nav-btn{background:transparent; border:2px solid rgba(255,255,255,0.15); color:white; padding:8px 12px; border-radius:8px; cursor:pointer}
    .nav-btn.active{background:rgba(0,0,0,0.15)}
    ...
    .search{padding:8px 12px; border-radius:8px; border:1px solid #e6e9ef; width:260px}

    .grid{display:grid; grid-template-columns:repeat(auto-fill,minmax(260px,1fr)); gap:14px}
    .card{background:var(--card); padding:14px; border-radius:12px; box-shadow:0 6px 18px rgba(39,40,45,0.06); display:flex; flex-direction:column; gap:8px; transition: background 0.3s, color 0.3s}
    ...
    .body{font-size:0.94rem; color:var(--text)}

    .actions{display:flex; gap:8px; margin-top:8px}
    button.action{flex:1; padding:8px 10px; border-radius:8px; border:1px solid #e6e9ef; background:transparent; cursor:pointer}
    button.action.like.active{border-color:#34d399; box-shadow:0 6px 14px rgba(52,211,153,0.12)}
    button.action.dislike.active{border-color:#f87171; box-shadow:0 6px 14px rgba(248,113,113,0.12)}
    button.spam{background:#fdf2f2; color:#991b1b; border:1px solid #f5c2c2}

    .empty{padding:40px; text-align:center; color:var(--muted); background:linear-gradient(180deg,#fff,#fbfbfb); border-radius:12px}
    [data-theme='dark'] .empty{background:var(--card)}

    .top-row{display:flex; gap:12px; align-items:center}
    .badge{background:#111827; color:white; padding:6px 8px; border-radius:999px; font-size:0.8rem}

    footer{max-width:1000px;margin:22px auto;padding:14px 16px;color:var(--muted); font-size:0.9rem}

    @media (max-width:480px){.controls{flex-direction:column; gap:8px} .search{width:100%}}
  </style>
</head>
